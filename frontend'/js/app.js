...
window.App = {
    ...
    VotingContract.deployed().then(function(instance){
      ...
      instance.getNumOfMotions().then(function(makeMotion){
        ...
        if (countMotions == 0){
          ...
          instance.makeMotion("Fund Chapters","Chapter Leader").then(function(result){
            $("#Motion-box").append(`<div class='form-check'><input class='form-check-input' type='checkbox' value='' id=${result.logs[0].args.proposerID}><label class='form-check-label' for=0>Fund Chapters</label></div>`)
          })
          instance.makeMotion("Create new coworking space","Board Member").then(function(result){
            $("#Motion-box").append(`<div class='form-check'><input class='form-check-input' type='checkbox' value='' id=${result.logs[0].args.proposerID}><label class='form-check-label' for=1>Fund Chapters</label></div>`)
          })
          ...
          for (var i = 0; i < countMotions; i++ ){
            // gets candidates and displays them
            instance.getMotion(i).then(function(data){
              $("#Motion-box").append(`<div class="form-check"><input class="form-check-input" type="checkbox" value="" id=${data[0]}><label class="form-check-label" for=${data[0]}>${window.web3.toAscii(data[1])}</label></div>`)
            })
          }
        }
        // sets global variable for number of Candidates
        // displaying and counting the number of Votes depends on this
        window.countMotions = countMotions
      })
    }).catch(function(err){
  ...
  vote: function() {
    ...
    if ($("#Motion-box :checkbox:checked").length > 0){
      // just takes the first checked box and gets its id
      var proposerID = $("#Motion-box :checkbox:checked")[0].id
    }
    else {
      // print message if user didn't vote for candidate
      $("#msg").html("<p>Please vote for a Motion.</p>")
      return
    }
    // Actually voting for the Candidate using the Contract and displaying "Voted"
    VotingContract.deployed().then(function(instance){
      instance.vote(name,parseInt(proposerID)).then(function(result){
        $("#msg").html("<p>Voted</p>")
      })
    }).catch(function(err){
      console.error("ERROR! " + err.message)
  ...
  findNumOfVotes: function() {
    VotingContract.deployed().then(function(instance){
      ...
      for (var i = 0; i < window.countMotions; i++){
        // calls two smart contract functions
        var candidatePromise = instance.getMotion(i)
